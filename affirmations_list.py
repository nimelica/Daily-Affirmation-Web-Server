...
cursor.execute("create table affirmations (id INTEGER PRIMARY KEY AUTOINCREMENT, name TEXT)")

# Fill that table
for i in range(len(affirmations)):
  cursor.execute("insert into affirmations (name) values (?)",[affirmations[i]])
